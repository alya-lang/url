...
WHATWG and RFC 3986 compliant URL parser, serializer, and query string library for Alya
...
| `new_config(name, count)` | `name = "World", count = 1` | `UrlConfig` | Constructs a new configuration struct. |
| `core_format_greeting(name)` | `name` | `string` | Core formatter producing `Hello, {name}!`. |
| `core_format_custom(config)` | `config: UrlConfig` | `string` | Formats greeting using prefix and name from config. |
